...
export async function GET() {
  try {
    ...
    }>(`SELECT metric, target, period FROM crm.performance_goals WHERE is_active = true`);
    ...
    // Today's calls from phone_calls table
...
export async function PATCH(request: NextRequest) {
  ...
  await query(
    `UPDATE crm.performance_goals SET target = $1, updated_at = NOW() WHERE metric = $2`,
    [target, metric]
  );
