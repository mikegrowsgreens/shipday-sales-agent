...
export async function GET() {
  try {
    // Overview stats
    const [overview] = await queryShipday<{
      total_deals: number;
      active_deals: number;
      total_drafts: number;
      total_sent: number;
      ...
      SELECT
        (SELECT COUNT(*) FROM shipday.deals WHERE agent_status IS NULL OR agent_status NOT IN ('archived')) AS total_deals,
        (SELECT COUNT(*) FROM shipday.deals WHERE agent_status = 'active') AS active_deals,
        (SELECT COUNT(*) FROM shipday.email_drafts) AS total_drafts,
        (SELECT COUNT(*) FROM shipday.email_drafts WHERE status = 'sent') AS total_sent,
        (SELECT COUNT(*) FROM shipday.email_drafts WHERE status = 'approved') AS total_approved,
        (SELECT COUNT(*) FROM shipday.email_drafts WHERE status = 'draft') AS total_pending
    `);

    // Sent count per touch number (response rates by touch)
    const touchStats = await queryShipday<{
      touch_number: number;
      total: number;
      sent: number;
      approved: number;
    ...
    const stageBreakdown = await queryShipday<{
      ...
      SELECT
        ...
        COUNT(*) FILTER (WHERE EXISTS (
          SELECT 1 FROM shipday.email_drafts ed WHERE ed.deal_id = d.deal_id
        )) AS with_campaign
    ...
    const recentActivity = await queryShipday<{
      action_date: string;
      action_count: number;
      sends: number;
      approvals: number;
    ...
    const completionStats = await queryShipday<{
      ...
      FROM (
        SELECT
          d.deal_id,
          (SELECT COUNT(*) FROM shipday.email_drafts ed WHERE ed.deal_id = d.deal_id) AS draft_total,
          (SELECT COUNT(*) FROM shipday.email_drafts ed WHERE ed.deal_id = d.deal_id AND ed.status = 'sent') AS sent_total
        FROM shipday.deals d
        WHERE d.agent_status IS NULL OR d.agent_status NOT IN ('archived')
      ) sub
    ...
    // Edited vs untouched drafts (how often Mike edits AI drafts)
    const editStats = await queryShipday<{
      edited_count: number;
