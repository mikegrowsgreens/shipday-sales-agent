...
export async function POST(request: NextRequest) {
  try {
    ...
    const deal = await queryShipdayOne<{ contact_email: string; contact_name: string }>(
      `SELECT contact_email, contact_name FROM shipday.deals WHERE deal_id = $1`,
      [deal_id],
    );
    ...
    if (store && emailData.context_summary) {
      await queryShipday(
        `UPDATE shipday.deals SET action_items = COALESCE(action_items, '') || E'\n\n--- EMAIL HISTORY ---\n' || $1, updated_at = NOW() WHERE deal_id = $2`,
