...
/**
 * POST /api/brain/sync
 * Aggregates live sales intelligence from wincall_brain tables
...
Use these stats to build credibility: "We've helped ${d.won}+ businesses optimize their delivery operations" and "Our average customer invests $${d.avg_won_mrr}/month and sees ROI within the first week."`,
        ...
        ['Uncertainty about whether Shipday works for their type of business']
      );
...
USAGE: When a prospect mentions a pain point from this list, acknowledge it as something you hear frequently — it builds trust and shows you understand their industry. For objections, reference how other restaurants overcame the same concern. For competitors, use the context to position Shipday's advantages.`;
...
USAGE: Mirror these value prop frames when matching prospect pain to Shipday solutions. Use closing moves when the prospect is showing buying signals. High-effectiveness frames should be prioritized. Focus on closing moves with "accepted" outcomes.`;
 ...
 * If an entry with that content_type already exists, update it.
...
async function upsertBrainContent(
  ...
  const existing = await query<{ id: string }>(
    `SELECT id FROM brain.internal_content WHERE content_type = $1 AND title = $2 LIMIT 1`,
    [contentType, title]
  );
  ...
  if (existing.length > 0) {
    await query(
      `UPDATE brain.internal_content
       ...
       WHERE id = $6`,
      [rawText, JSON.stringify(keyClaims), JSON.stringify(valueProps), JSON.stringify(painPoints), contentHash, existing[0].id]
    );
  } else {
    await query(
      `INSERT INTO brain.internal_content
       ...
       VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7, 'automated', true, NOW(), NOW())`,
      [contentHash, contentType, title, rawText, JSON.stringify(keyClaims), JSON.stringify(valueProps), JSON.stringify(painPoints)]
    );
  }
}
